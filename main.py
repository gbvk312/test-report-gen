...
def main():
    parser = argparse.ArgumentParser(
        description="Generate HTML Test Reports from various log formats."
    )
    parser.add_argument("--pytest-logs", type=str, help="Path to pytest log file")
    parser.add_argument(
        "--automation-logs", type=str, help="Path to generic automation log file"
    )
    parser.add_argument(
        "--network-logs", type=str, help="Path to network test log file"
    )
    parser.add_argument(
        "--output", type=str, default="report.html", help="Path for output HTML report"
    )

    args = parser.parse_args()

    # Collect data from all provided logs
    combined_data = {"total": 0, "passed": 0, "failed": 0, "details": []}
    ...
    if files_processed == 0:
        print(
            "Error: Please provide at least one log file (--pytest-logs, --automation-logs, or --network-logs)."
        )
        parser.print_help()
        sys.exit(1)

    print(
        f"Total Tests Found: {combined_data['total']} (Passed: {combined_data['passed']}, Failed: {combined_data['failed']})"
    )
